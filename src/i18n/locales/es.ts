import type { Translations } from '../translations';

const es: Translations = {
  title: 'Matriz de Eisenhower',
  subtitle: 'Organiza tus tareas por prioridad y urgencia',

  quadrants: {
    urgentImportant: {
      title: 'Urgente e Importante',
      description: 'Hacer inmediatamente',
    },
    notUrgentImportant: {
      title: 'Importante pero No Urgente',
      description: 'Programar',
    },
    urgentNotImportant: {
      title: 'Urgente pero No Importante',
      description: 'Delegar',
    },
    notUrgentNotImportant: {
      title: 'Ni Urgente ni Importante',
      description: 'Eliminar',
    },
  },

  axes: {
    important: 'IMPORTANTE',
    notImportant: 'NO IMPORTANTE',
    urgent: 'URGENTE',
    notUrgent: 'NO URGENTE',
  },

  tasks: {
    noTasks: 'Sin tareas',
    addTask: 'Añadir tarea',
    enterTask: 'Escribe una tarea...',
    add: 'Añadir',
    cancel: 'Cancelar',
    editTask: 'Editar tarea',
    deleteTask: 'Eliminar tarea',
    completeTask: 'Completar tarea',
  },

  theme: {
    toggle: 'Cambiar tema',
  },

  dialogs: {
    deleteConfirmTitle: '¿Eliminar tarea?',
    deleteConfirmDescription: 'Esta acción no se puede deshacer.',
    confirm: 'Eliminar',
    cancel: 'Cancelar',
  },

  toasts: {
    taskAdded: 'Tarea añadida',
    taskDeleted: 'Tarea eliminada',
    taskMoved: 'Tarea movida',
    taskCompleted: 'Tarea completada',
    error: 'Ocurrió un error',
  },

  errorBoundary: {
    title: 'Algo salió mal',
    description: 'Ocurrió un error inesperado',
    retry: 'Intentar de nuevo',
  },

  states: {
    loading: 'Cargando...',
    error: 'Error',
    retry: 'Reintentar',
  },

  archive: {
    title: 'Tareas archivadas',
    noTasks: 'No hay tareas archivadas',
    completedOn: 'Completada el',
    deleteForever: 'Eliminar permanentemente',
    backToMatrix: 'Volver a la matriz',
    openArchive: 'Abrir archivo',
    searchPlaceholder: 'Buscar tareas archivadas',
    allQuadrants: 'Todos los cuadrantes',
    applyFilters: 'Aplicar',
    restoreTask: 'Restaurar tarea',
    previousPage: 'Página anterior',
    nextPage: 'Página siguiente',
    pagePlaceholder: 'Página',
    goToPage: 'Ir',
  },

  sessions: {
    title: 'Sesiones activas',
    openSessions: 'Abrir sesiones',
    loading: 'Cargando sesiones...',
    backToMatrix: 'Volver a la matriz',
    revokeOtherSessions: 'Revocar otras sesiones',
    revokingOthers: 'Revocando...',
    noSessions: 'No hay sesiones activas.',
    unknownDevice: 'Dispositivo desconocido',
    unknownIp: 'Desconocida',
    current: 'Actual',
    createdAt: 'Creada',
    lastSeenAt: 'Última actividad',
    revokeSession: 'Revocar sesión',
  },

  accessibility: {
    selectLanguage: 'Seleccionar idioma',
    dismissNotification: 'Cerrar notificación',
  },

  auth: {
    signInTitle: 'Iniciar sesión en Eisenhower Board',
    signInSubtitle: 'Ingresa tu email y te enviaremos un enlace seguro de inicio de sesión.',
    emailLabel: 'Email',
    sendMagicLink: 'Enviar enlace mágico',
    sendingLink: 'Enviando enlace...',
    checkInbox: 'Revisa tu bandeja de entrada',
    checkInboxDescription: 'Enviamos un enlace de inicio de sesión a',
    linkExpiresIn: 'El enlace expira en 15 minutos.',
    signOut: 'Cerrar sesión',
    signedInAs: 'Conectado como',
    sessionCheckFailedTitle: 'No se pudo verificar tu sesión',
    sessionCheckFailedDescription: 'Comprueba tu conexión e inténtalo de nuevo antes de iniciar sesión.',
    retrySessionCheck: 'Reintentar',
  },

  landing: {
    description: 'La Matriz de Eisenhower es un método de productividad que te ayuda a organizar y priorizar tareas clasificándolas en cuatro cuadrantes según su urgencia e importancia. Concéntrate en lo que realmente importa y deja de perder tiempo en distracciones.',
    quadrantsTitle: 'Cuatro cuadrantes para organizar tus prioridades',
  },

  admin: {
    title: 'Administración',
    stats: 'Estadísticas',
    totalUsers: 'Usuarios en total',
    activeUsers30d: 'Usuarios activos (30 días)',
    users: 'Usuarios',
    email: 'Email',
    createdAt: 'Creado',
    lastLoginAt: 'Último inicio de sesión',
    taskCount: 'Tareas',
    never: 'Nunca',
    deleteUser: 'Eliminar usuario',
    deleteUserConfirmTitle: '¿Eliminar usuario?',
    deleteUserConfirmDescription: 'Todos los datos de {email} serán eliminados permanentemente. Esta acción es irreversible.',
    cannotDeleteSelf: 'No puedes eliminar tu propia cuenta desde esta página',
    userDeleted: 'Usuario eliminado',
  },

  account: {
    title: 'Cuenta',
    currentEmail: 'Email actual',
    newEmail: 'Nuevo email',
    changeEmail: 'Cambiar email',
    changingEmail: 'Modificando...',
    emailChangeRequested: 'Email de verificación enviado',
    emailChangeRequestedDescription: 'Revisa la bandeja de entrada de tu nuevo email para confirmar el cambio.',
    emailChanged: 'Email modificado',
    emailChangedDescription: 'Tu email ha sido actualizado con éxito.',
    dangerZone: 'Zona de peligro',
    deleteAccount: 'Eliminar mi cuenta',
    deleteAccountConfirmTitle: '¿Eliminar tu cuenta?',
    deleteAccountConfirmDescription: 'Todas tus tareas y datos serán eliminados permanentemente. Esta acción es irreversible.',
    deleteAccountTypeEmail: 'Escribe tu email para confirmar',
    deletingAccount: 'Eliminando...',
    accountDeleted: 'Cuenta eliminada',
  },
};

export default es;
