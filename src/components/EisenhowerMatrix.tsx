...
const QUADRANT_CONFIGS: Record<QuadrantKey, QuadrantConfig> = {
  urgentImportant: {
      ...
      <div className="space-y-4">
        <div className="relative grid grid-cols-1 gap-4 md:ml-16 md:grid-cols-2">
          ...
          <div className="absolute -top-8 left-1/4 hidden -translate-x-1/2 md:block">
            <span className="tracking-wider text-slate-700 dark:text-slate-300">URGENT</span>
          </div>
          <div className="absolute -top-8 right-1/4 hidden translate-x-1/2 md:block">
            <span className="tracking-wider text-slate-700 dark:text-slate-300">NON-URGENT</span>
          </div>

          {/* Quadrants */}
          {(Object.keys(QUADRANT_CONFIGS) as QuadrantKey[]).map((key) => (
            <QuadrantCard
              key={key}
              quadrantKey={key}
              config={QUADRANT_CONFIGS[key]}
              tasks={quadrants[key]}
